...
def get_and_refresh_token():
    """
    Fetches the Meta Token from Supabase. 
    If it's older than 45 days, it automatically refreshes it.
    """
    # 1. Get current token from Supabase
    row = supabase.table("app_config").select("*").eq("key", "meta_token").execute().data
    if not row: return None
    
    current_token = row[0]['value']
    last_updated = row[0]['updated_at'] # String format: 2024-01-01T12:00:00+00:00

    # 2. Check Age (Simple Logic)
    # We parse the date to see if it's been > 45 days
    last_date = datetime.fromisoformat(last_updated.replace('Z', '+00:00'))
    days_old = (datetime.now(last_date.tzinfo) - last_date).days

    if days_old > 45:
        # 3. Refresh It!
        st.toast("🔄 Refreshing Meta Token...", icon="🛡️")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                }).eq("key", "meta_token").execute()
                return new_token
            else:
                st.error(f"Token Refresh Failed: {r.text}")
